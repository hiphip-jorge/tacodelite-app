...
describe('MenuService Cache Management', () => {
    ...
    describe('Cache Manager', () => {
        it('should clear all caches when clearAll is called', () => {
            ...
            localStorage.setItem('menu_items', JSON.stringify([{ id: 1, name: 'Test' }]))
            localStorage.setItem('menu_categories', JSON.stringify([{ id: 1, name: 'Test' }]))
        ...
        it('should provide cache info for debugging', () => {
            // Set some test data in localStorage
            const testMenuItems = [{ id: 1, name: 'Test Item', price: 1.00, categoryId: 1 }]
            const testCategories = [{ id: 1, name: 'Test Category', pk: 'CATEGORY#001' }]
            ...
            localStorage.setItem('menu_categories', JSON.stringify({
                data: testCategories,
                etag: 'def456',
                timestamp: Date.now()
            }))
            localStorage.setItem('menu_version', JSON.stringify({
                data: '1',
                timestamp: Date.now()
            }))
